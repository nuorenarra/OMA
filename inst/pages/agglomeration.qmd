...
family or genus of each species. This means you would add up the counts of all species within
a particular family to get a single value that represents that family. While
this method simplifies your dataset, it means you lose the specific information 
about individual species. It can, however, be useful in and highlighting overall trends.

The choice between subsetting (samples or features) and agglomeration (samples of features) 
...
(e.g., gut microbiome and soil microbiome),you might want to first examine/plot their taxonomic 
profiles at the level of Phyla, of Families, rather than species, to get an idea of their 
...
focus on broader patterns and trends in the community while managing the complexity of the
dataset. Similarly, in addition to aggregating features to higher categories, you  might want 
to aggregate samples within the give sample type, to examine/plot these broad differences across sample 
...
One of the main applications of taxonomic information in sequnencing data
is to agglomerate counts (such as ASV counts) to specific taxonomic levels 
and track the influence of changing conditions through these levels. For this `mia` contains the
...
that is, the agglomerates are made based on the feature data, such as taxonomy
information. Alternatively, you can also use `agglomerateByVariable()`
to aggregate the data based on `rowData`. In this case the agglomerates 
are made based on the sample variables. 
...
Note that a dataset might contain taxa with the same lower-level rank, even
