...
it, any type of microbiome data analysis will make little sense. However,
...
a short introduction on the methods available without ranking one over the
other. This has to be your choice based on your individual dataset.
...
row of data and a training dataset based on a taxonomic reference database 
(such as Silva or Green Genes). For more information visit the
...
either taxonomic or functional information on the sequence variants. For `mia`,
we are using only the first, taxonomic, option for now. More information can be
...
This can then be used to subset the `rowData` to focus only on the taxonomy-related columns.
...
logical vector of `length(x)`  (where `length(x)==ncol/rowData(tse)[, taxonomyRanks(tse)]`).
...
```{r}
getTaxonomyLabels(tse[rank_found,], with.rank = TRUE, make.unique = FALSE) |> head()
```
...
Subsetting data based on excluding certain taxa or samples is explained in detail 
in [@sec-treese_subsetting]. However, if you've already subsetted your data, you 
may have noticed that the taxonomy tree does not automatically update when using 
the `[]` subsetting operators. Although the linkages between rows and tree nodes 
remain correct, the tree retains its original, complex structure. You may be 
...
We see that even though we have only 5 rows, the tree still retains its original number of
tips. To update the tree by aligning it with the subsetted data, we can use the
