...
`splitOn()`. It works for row-wise and column-wise splitting, that is for taking 
a subset of features/taxa (rows) or samples (columns). Splitting the data may be useful, 
for example, if you want to analyze different sample types separately:
...
## Add or modify sample variables {#sec-add-or-modify-data}

The information about samples contained by the `colData` of a `TreeSE` can be added and/or 
modified. You might want to add or modify this data to include new sample variables 
or update existing ones, to ensure all relevant metadata is available for subsequent analyses.

```{r}
#| label: modify_coldata

# modify the entries of sample variable "Description"
...
New information can be added to the data by creating a new variable.

```{r}
#| label: add-coldata

# simulate values for a new variable
new_variable <- runif(ncol(tse))

# store new data as new variable in colData
colData(tse)$NewVariable <- new_variable

# view new variable
tse$NewVariable |> head()
```

Alternatively, you can add a whole table of sample information 
into the `TreeSE` object by merging it with existing `colData`.

```{r}
#| label: add-coldata2

# simulate a table of new data with multiple columns
new_data <- data.frame(var1 = runif(ncol(tse)), var2 = runif(ncol(tse)))
#name the ros of the table according to the sample names of your data (colnames)
...
Similar steps can also be applied to `rowData`, i.e., you can add and modify the variables 
describing your features/taxa. If you have an assay whose
...
features to your existing dataset, you can use `cbind()` to add columns for
...
`SummarizedExperiment` objects. As an example, we will be merging
multiple `TreeSE` objects each including one sample.
...
# Derive data subsets with one sample each for demonstration purposes
tse1 <- tse[, 1]
tse2 <- tse[, 2]
tse3 <- tse[, 3]
# Data subset with first 100 features and one sample:
tse4 <- tse[1:100, 4]
```
...
# We use join = "inner" to include all shared rows into the merged object. 
# Note when using mergeSEs function all samples are always preserved.
...
# join = "left" preserves all rows of the 1st object
# missing.values = 0 ensures that all missing values in the assay are replaced with a 0
...
`meltSE()`. a long `data.frame` has one row per each combination of feature and sample.
...
For `MultiAssayExperiment` data, you can use `longFormat()` instead of `meltSE()`.
